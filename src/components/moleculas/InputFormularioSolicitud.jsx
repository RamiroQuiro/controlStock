...
export default function InputFormularioSolicitud({ name, placeholder,type, id, children ,onchange,value ,className, isMoney,disabled}) {
    ...
    return (
        <div className="relative w-full  group">
            ...
            <input disabled={disabled} placeholder={placeholder} type={type} name={name} id={id} value={value} onChange={onchange} className={` ${className} disabled:bg-transparent  peer  disabled:p-0  bg-primary-100/15 focus:ring focus:border-transparent peer z-10 w-full disabled:px-1  left-0 px-1 py-1 text-sm rounded-md valid:ring-0   outline-none   focus:outline-none ${isMoney && 'text-end'}` }/>
            {type === 'password' && (
