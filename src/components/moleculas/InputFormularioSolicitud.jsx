...
export default function InputFormularioSolicitud({ name, type, id, children ,onchange,value , isMoney}) {
    ...
    return (
        <div className="relative w-full my-2 group">
            ...
            <input type={type} name={name} id={id} value={value} onChange={onchange} className="bg-gray-100 peer z-10 w-full  left-0 px-1 py-1 text-sm rounded shadow valid:ring-0   outline-none   focus:outline-none" />
        </div>
    )
}
