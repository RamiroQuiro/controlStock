...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!razonSocial || !documento) {
      return new Response(
        JSON.stringify({ error: "Razón Social y Documento son obligatorios" }),
        { status: 400 }
      );
    }
    ...
    if (!updatedEmpresa) {
      return new Response(
        JSON.stringify({ error: "Empresa no encontrada al actualizar" }),
        { status: 404 }
      );
    }
...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  try {
    const empresaData = (
      await db.select().from(empresas).where(eq(empresas.id, queryEmpresa))
    ).at(0);
...
export const PUT: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    console.log("enpoint de put -> data", data);
