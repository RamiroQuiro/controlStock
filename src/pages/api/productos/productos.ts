...
export const DELETE: APIRoute = async ({ request }) => {
  ...
  if (!productoId) {
    return new Response(JSON.stringify({ msg: "ID de producto requerido" }), {
      status: 400,
    });
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ status: 200, msg: "Producto eliminado" }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }
    ...
    return new Response(
      JSON.stringify({ status: 500, msg: "Error al eliminar el producto" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
...
export const PUT: APIRoute = async ({ request, locals }) => {
  ...
  if (!productoId || !empresaId) {
    return new Response(
      JSON.stringify({
        msg: "ID de producto y autenticación de empresa requeridos",
      }),
      { status: 400 }
    );
  }
  ...
  try {
    const productoActualizado = await updateProductoInDB(
      productoId,
      data,
      empresaId
    );
