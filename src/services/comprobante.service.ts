...
export class ComprobanteService {
  ...
  public async generarTicketHTML2(data: {
    id: string;
    fecha: string;
    codigo: string;
    cliente: {
  ...
  public async generarTicketHTML(data: ComprobanteDetalle) {
    // Estilos específicos para la impresión de tickets
    const styles = `
      <style>
        @media print {
          body {
            font-family: 'Courier New', Courier, monospace;
            width: 80mm; /* Ancho estándar para impresoras de tickets */
            margin: 0;
            padding: 5px;
            font-size: 12px;
            color: #000;
          ...
          .total-row td { font-size: 12px; font-weight: bold; }
        }
        /* Ocultar en la vista normal del navegador */
        @media screen {
          body { display: none; }
        }
      </style>
    ...
    const itemsHTML = data.items.map(item => `
      <tr class="item-row">
        ...
        <td class="text-right">${formateoMoneda.format(item.precioUnitario)}</td>
        <td class="text-right">${formateoMoneda.format(item.cantidad * item.precioUnitario)}</td>
      </tr>
    `).join('');
        ...
        <body>
          <div class="ticket-container">
            <div class="text-center mb-2">
              <h2 class="font-bold">${data.dataEmpresa?.razonSocial || ''}</h2>
              <p class="text-xs">${data.dataEmpresa?.direccion || ''}</p>
              <p class="text-xs">CUIT: ${data.dataEmpresa?.documento || ''}</p>
              <p class="text-xs">Tel: ${data.dataEmpresa?.telefono || ''}</p>
            </div>

            <div class="mb-2 border-t border-b">
              <p class="text-sm">Fecha: ${formatDate(data.fecha)}</p>
              <p class="text-xs">ID Venta: ${data.id}</p>
              <p class="text-sm">Comprobante: ${data.comprobante.numeroFormateado}</p>
            </div>

            <div class="mb-4">
              <p class="text-sm font-bold">Cliente: ${data.cliente?.nombre || 'Consumidor Final'}</p>
              <p class="text-xs">DNI: ${data.cliente?.dni || '-'}</p>
              ${data.cliente?.direccion ? `<p class="text-xs">Dir: ${data.cliente.direccion}</p>` : ''}
            </div>
            ...
            <div class="border-t pt-1 mb-1">
              <div class="flex justify-between">
                ...
                <span class="text-sm">${formateoMoneda.format(sumaSubtotal)}</span>
              </div>
              ${data.totales?.descuento > 0 ? `
              <div class="flex justify-between">
                <span class="text-sm">Descuento:</span>
                <span class="text-sm">-${formateoMoneda.format(data.totales.descuento)}</span>
              </div>
              ` : ''}
              <div class="flex justify-between">
                <span class="text-sm">IVA:</span>
                <span class="text-sm">${formateoMoneda.format(sumaTotal - sumaSubtotal)}</span>
              </div>
              <div class="flex justify-between font-bold border-t pt-1">
                <span class="text-base">TOTAL:</span>
                <span class="text-base">${formateoMoneda.format(data.totales.total)}</span>
              </div>
            </div>
